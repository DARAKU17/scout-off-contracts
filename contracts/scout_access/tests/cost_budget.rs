...
// #1040: get_player_access_grants reads at most two fixed-size index pages
// plus one grant record per returned entry (see ACCESS_GRANT_PAGE_SIZE /
// MAX_ACCESS_GRANT_PAGE_LIMIT in lib.rs), so its cost must stay flat
// regardless of how many grants the player has accumulated in total.
const GET_PLAYER_ACCESS_GRANTS_CPU_BUDGET: u64 = 20_000_000;
