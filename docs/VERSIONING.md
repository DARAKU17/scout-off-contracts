...
| v0.3.0 (scout_access) | 2026-08-18 | MINOR | Implemented `EvidenceAccessGrant` confidential-evidence authorization (#1040): new type, atomic write in `pay_to_contact`/`batch_contact_players`, `has_evidence_access`/`get_evidence_access_grant`/`get_player_access_grants` query API, admin-gated `admin_revoke_evidence_access`, `evidence_access_granted`/`evidence_access_revoked` events, 1 error code appended (`GrantNotFound` 29) |
