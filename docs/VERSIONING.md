...
| v1.0.0 (scout_access) | 2026-08-19 | MAJOR | BREAKING: `health()` return type `ContractHealth` gains a `pay_to_contact_paused: bool` field (function-scoped pause for `pay_to_contact`, issue #1056). `ContractHealth` is constructed identically on `registration`/`verification`/`progress` (`pay_to_contact_paused: false`), so callers of those contracts must rebuild bindings but observe no behavior change. Also appended error code `PayToContactPaused` (30) and the `pay_to_contact_paused`/`pay_to_contact_unpaused` events — additive, non-breaking |
