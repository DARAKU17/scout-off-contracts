...
| v0.4.0 (verification) | 2026-08-19 | MAJOR | BREAKING: `dispute_milestone` gains `impact_score: u32` parameter (signature change); `MilestoneDispute` struct gains 6 new fields (`impact_score`, `jury_required`, `quorum`, `voting_deadline`, `votes_for`, `votes_against`). Added `set_jury_config`, `get_jury_config`, `cast_dispute_vote`, `tally_dispute`; 7 new error codes appended (32–38); 2 new events (`dispute_vote_cast`, `dispute_tallied`). `resolve_dispute` now returns `DisputeRequiresJury` for jury-routed disputes. See CHANGELOG.md for migration guide |
